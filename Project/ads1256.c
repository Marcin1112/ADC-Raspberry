...
int  main(int argc, char* argv[])
{
   ...
   printf("ID=\r\n");  
	...
	char* SPS2d5 = "-2.5SPS";
	if(argc==2){
		if(strcmp(argv[1], SPS5)==0){
			ADS1256_CfgADC(ADS1256_GAIN_1, ADS1256_5SPS);
			prescaler = 8.0/5.0;
		}else if(strcmp(argv[1], SPS2d5)==0){
			ADS1256_CfgADC(ADS1256_GAIN_1, ADS1256_2d5SPS);
			prescaler = 8.0/2.5;
		}else if(strcmp(argv[1], SPS100)==0){
			ADS1256_CfgADC(ADS1256_GAIN_1, ADS1256_100SPS);
			prescaler = 8.0/100;
		}else if(strcmp(argv[1], SPS50)==0){
	...
	fclose(fopen("measurementData.dat", "w")); // clear the data
	FILE *log = NULL;
	log = fopen("measurementData.dat", "a");
	if (log == NULL)
    {
	...
	while(1)
	{
		...
		if(loopNumber==8){
			fprintf(log, "%lf;", measurementTime); 
			for (i = 0; i < ch_num; i++)
			{
				 fprintf(log, "%ld.%03ld%03ld;", volt[i] /1000000, (volt[i]%1000000)/1000, volt[i]%1000);  
				 fflush(log);  
			}
			measurementTime += 1.0*prescaler;
			fprintf(log, "\r\n");
